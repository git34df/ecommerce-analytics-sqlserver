...
frecuencia AS (
    -- ✅ JOIN a dim_customers eliminado: innecesario
    SELECT
        customer_id,
        COUNT(*) AS total_ordenes
    FROM fact_orders
...
monetario AS (
    -- ✅ Fuente corregida: fact_orders, no dim_customers
    --    El gasto es un hecho — no pertenece a la dimensión
    SELECT
        customer_id,
...
rfm_scores AS (
    SELECT
        ...
        -- ✅ PERCENT_RANK en lugar de NTILE
        --    NTILE distribuye filas mecánicamente aunque todas tengan
        --    el mismo valor — genera scores inflados con datos homogéneos.
        --    PERCENT_RANK calcula la posición relativa real del cliente.

        -- Recencia: menos días desde la última compra = mejor cliente
        CASE
        ...
        -- Frecuencia: más órdenes = mejor cliente
        CASE
        ...
        -- Monetario: más gasto acumulado = mejor cliente
        CASE
